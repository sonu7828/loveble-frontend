// Staff-callable: send pre-op (before-visit) instructions email for an appointment.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: appt, error: ae } = await supa
      .from("appointments")
      .select("id, client_first_name, client_email, service_id, pre_op_sent_at, services(name)")
      .eq("id", appointmentId)
      .maybeSingle();
    ...
    const { data: preOps } = await supa
      .from("service_pre_op_instructions")
      .select("service_id, title, body_markdown")
      .in("service_id", serviceIds);
    const byId = new Map((preOps ?? []).map((p: any) => [p.service_id, p]));
    ...
    if (!blocks.length) return json({ ok: true, skipped: "no_pre_op_configured" });

    const serviceName = blocks.map((b) => b.name).join(" + ");
    const title = blocks.length === 1 ? blocks[0].title : "Pre-Treatment Instructions";
    ...
    const { error: ee } = await supa.functions.invoke("send-transactional-email", {
      body: {
        templateName: "pre-op-instructions",
        recipientEmail: appt.client_email,
        idempotencyKey: force ? `pre-op-${appointmentId}-${Date.now()}` : `pre-op-${appointmentId}`,
        templateData: {
    ...
    await supa.from("appointments").update({ pre_op_sent_at: new Date().toISOString() }).eq("id", appointmentId);
