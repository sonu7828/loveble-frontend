UPDATE public.consent_forms SET is_optional = true
WHERE slug IN ('marketing-authorization','Marketing consent','Model Release','Photo/video release')
   OR lower(title) LIKE '%marketing%'
   OR lower(title) LIKE '%model release%'
   OR lower(title) LIKE '%photo/video release%';
